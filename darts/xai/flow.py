...
class CreditFlow:
    ...
    def __init__(self, graph, verbose=False, nruns=10,
        ...
        verbose: wheahther to print out decision process
    ...
    def credit2dot_pygraphviz(self, edge_credit, format_str, idx=-1,
        ...
        self.rankdir = self.rankdir if hasattr(self, "rankdir") else "TB"
        G = AGraph(directed=True, rankdir=self.rankdir, splines='true')
        G.node_attr["style"] = "filled"
        G.node_attr["shape"] = "circle"
        G.node_attr["fixedsize"] = "true"
        G.node_attr["fontname"] = "Computer Modern"
        G.edge_attr["spline"] = "spline"
        edge_values = []
                ...
                for node in [node1, node2]:
                    if node not in G:
                        if node.is_noise_node and self.fold_noise:
                            G.add_node(node, shape="rectangle")
                        else:
                            shape = 'box' if node.is_categorical else 'circle'
                            if idx < 0 or \
        ...
        def set_node_x_format(n_):
            n = G.get_node(n_)
            if not 'noise' in n.name:
                idx = int(n.name.split('_', 1)[1])
                if idx < 0:
                    print(idx)
                    G.delete_node(n_) 
                    return
                n.attr["height"] = "%s" % (1)
                n.attr["width"] = "%s" % (1)
                n.attr["pos"] = str(idx*2)+","+str(0)+"!"
            else:
                idx = int(n.name.split(' ', 1)[0].split('_', 1)[1])
                n.attr["shape"] = "rectangle"
                n.attr["pos"] = str(idx*2)+","+str(-1)+"!"

        # G.nodes() doesn't work
        nodes = []
        for n1, d in edge_credit.items():
            for n2, v in d.items():
                nodes.append(n1)
                nodes.append(n2)
        
        nodes = sorted(list(set(nodes)), key = lambda n: n.name)
...
def create_ar_f(parents, m, **kwargs):

    def args_to_ts(*args):
        ts_list = []
        df_ = pd.DataFrame.from_dict({n: args[i] for i, n in enumerate(parents)})
        df_ = df_.T
        for c in df_.columns:
            ts_list.append(TimeSeries.from_series(df_[c].reset_index().drop('index', axis=1), freq='H'))

        return ts_list

    def f_(*args):
        bs_length = len(args[0])
        o = np.zeros(bs_length)
        ts_list = args_to_ts(*args)
        for i in range(len(ts_list)):
            o[i] = m.model.apply(ts_list[i].values()).forecast(1)
        return o
